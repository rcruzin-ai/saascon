...
describe("history aggregation — last 7 days", () => {
  ...
  it("returns exactly 7 rows on an empty DB, all zero", async () => {
    const days = await queries.getHistoryLastNDays(7);
    expect(days).toHaveLength(7);
    expect(days.every((d) => d.calories === 0)).toBe(true);
  });

  it("aggregates by local day and fills gaps with zeros", async () => {
  ...
  it("excludes entries older than the N-day window", async () => {
  ...
  it("orders newest-first (today is index 0)", async () => {
  ...
  it("treats null macros as zero in the sum", async () => {
    ...
    const today = (await queries.getHistoryLastNDays(7))[0];
